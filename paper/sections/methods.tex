...
procedure (Fig.~\ref{fig:selection-cost}c).
...
\emph{one-sided}: one stored summary is matched against a query predicate, and a surviving block is a
\emph{candidate} that must then be rechecked, because the summary is a lossy approximation of its
block. The test used here is \emph{two-sided} --- the summaries of both operands are combined,
$\mathrm{occ}_A \wedge \mathrm{occ}_B$ --- and its popcount is not a candidate count but the exact
number of bins the pairing will visit, because a bin absent from either operand contributes nothing
to the intersection by construction. There is no recheck and no false-positive rate to trade against
space. Second, the bins partition \emph{bit position} rather than a value domain, so no histogram is
...
\paragraph*{Refinements available from the same metadata.} The summary carries one bit per bin, and
the two tightenings below need only wider per-bin metadata, not a different mechanism. Storing the
per-bin cardinalities $a_k,b_k$ instead of occupancy alone gives
$|A \cap B| \le \sum_k \min(a_k,b_k)$, which is sound and never weaker than the interval bound of
Eq.~\eqref{eq:interval}, since $\sum_k \min(a_k,b_k) \le \min(a,b)$; storing the first and last set
position within each bin admits a second, near-free rejection --- a bin survives only if
$\max(\mathrm{first}_A,\mathrm{first}_B) \le \min(\mathrm{last}_A,\mathrm{last}_B)$ --- which is the
qualify/disqualify test of \cite{moerkotte1998sma} applied per bin rather than per block.
\spec{Both are stated here as available consequences of the same metadata; neither is measured in
this paper, and the storage they add is charged against the budget of
Supplementary Table~\ref{tab:metadata}.} \nm{measured effect of per-bin count and first/last
refinements, or a statement that they were not evaluated}

\paragraph*{A hierarchy over bins does not help, and the reason is general.} The obvious next step
--- a second summary level over groups of bins, as BitFunnel's higher ranks provide
\cite{goodwin2017bitfunnel} --- was implemented and measured
\prov{2.4 to 100 times slower than the flat structure across six corpora}. The cause is not the
extra indirection but a redundancy: once occupancy is materialised as a compact list of the bins that
are actually occupied, every empty and singleton bin has already been eliminated, so a coarse level
has nothing left to prune and adds only a dense scan over all coarse bins. A hierarchy exists to
avoid visiting empty regions; a list of occupied entries never visits them at all. The result is
therefore conditional on that representation rather than on the universe size, and it does not
exclude a hierarchy over a \emph{dense} summary at universes large enough that the flat AND is itself
expensive \draftnote{RESEARCH\_PLAN 13.2 records this case as unbuilt; keep the claim scoped to the
compact-list representation actually measured}. The cost model for this filter, the crossings that bound where
...
scalar merge loses to a bitset--bitset popcount for a count-only AND is
a cardinality of \prov{64--128}; the pass uses a default of
\prov{512} to leave margin for non-uniform data.
