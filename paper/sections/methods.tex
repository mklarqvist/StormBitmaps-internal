...
$\Theta(\ell + f)$ against a bitmap, where $\ell$ is the literal-word count and $f$ the fill-word
...
with a rank index; $\cell{\rB}{\rW}=\Theta(\ell+f)$; $\cell{\rS}{\rS}=\Theta(|A|+|B|)$ by merge or
$\Theta(|A|\log(|B|/|A|))$ by galloping search when $|A|\ll|B|$; $\cell{\rS}{\rR}=\Theta(|S|+r)$ by
merge or $\Theta(|S|\log r)$ by binary search into runs; $\cell{\rS}{\rW}=\Theta(|S|+\ell+f)$;
$\cell{\rR}{\rR}=\Theta(r_A+r_B)$ by run merge; $\cell{\rR}{\rW}=\Theta(r_A+(\ell+f)_B)$; and
$\cell{\rW}{\rW}=\Theta((\ell+f)_A+(\ell+f)_B)$. Every cell except $\cell{\rB}{\rB}$ is sub-linear in
the universe size $m$.
...
Each cell is implemented in its own source file (\texttt{cell\_bb.cpp}, \texttt{cell\_bs.cpp},
\texttt{cell\_br.cpp}, \texttt{cell\_sparse.cpp}, \texttt{cell\_wah.cpp}, \texttt{cell\_comp.cpp}),
against a shared representation layer (\texttt{storm\_repr.\{h,cpp\}}: \texttt{BitmapView},
\texttt{ListView}, \texttt{RunView}, \texttt{EwahView}, \texttt{ComplementView}). Several of the
asymmetric cells have more than one implemented strategy --- for example \cell{\rB}{\rR} has a
rank-indexed variant and a scalar per-run-scan variant --- and each variant was raced against the
independent oracle described below before being admitted to any comparison. The variant reported for
a given cell in Results is the one selected as fastest for the density regime or corpus shape being
reported, not a single fixed implementation chosen in advance; the full record of implemented variants,
including those that did not win any regime, is not itemized in this paper and is available in the
project repository.
...
specific to this setting is applying the same test as an \emph{exact} work-count for all-pairs
intersection cardinality, where the summary's own popcount is the number of bins that must be visited
rather than a heuristic estimate of it.
...
regret against that bound alone.
...
\nm{value of \texttt{DEFAULT\_MAX\_SIZE} in the pinned CRoaring version} (declared at
\texttt{roaring.h:2486} and tested at dispatch at \texttt{roaring.h:5515} in the pinned version
below). The same fixed commitment prices a run container against a bitset by iterating every run and
...
Every cell variant is checked against an independent scalar oracle (\texttt{oracle.cpp}) that computes
intersection cardinality by direct bit-by-bit or element-by-element counting, sharing no code path with
any of the variants under test, before that variant is admitted to any timed comparison; a variant that
returns a wrong cardinality is not reported as fast regardless of its measured speed. Correctness is
tracked at three levels: an ABI-level regression suite (\texttt{test\_storm.c}, \nm{test\_storm check
count} checks against the oracle, \nm{test\_storm failure count, expected "0"} failures) that is
compiled and linked as C to serve as the public C ABI's own regression test; a cell-level differential
suite (\texttt{test\_cells}, \nm{test\_cells check count} checks, \nm{test\_cells failure count,
expected "0"} failures) that exercises every implemented cell variant against the oracle across a grid
of densities, structures and alignments; and a larger differential run, on the order of
...
vendored at \texttt{third\_party/croaring\_amalg}, so that the exact baseline is reproducible rather
than referred to generically as ``CRoaring.''
