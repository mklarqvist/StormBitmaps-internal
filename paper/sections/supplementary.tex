...
$p(X) = |X| - \lceil t|X| \rceil + 1$ \cite{xiao2008ppjoin}, following the prefix-filtering
...
\suppnote{What repeats across a batch, and how each mechanism amortises it}
\label{note:batch}
...
\texttt{\seqsplit{chr20\_hap.bin}} & G3 (scoping) & \nm{n} & \prov{$\approx$1{,}048{,}576} & \nm{n} & \prov{3.1e-2} & haplotype-major; large universe but 3.1\% dense -- nothing beats all-bitmap (0.93$\times$) at this density and layout \\
\addlinespace
...
and zone map are constructed unconditionally, and both scale with universe, not
cardinality. Meta B/row and data B/row are bytes of selector metadata and bytes of
