...
set -uo pipefail
cd "$(dirname "$0")/.."
OUT=${1:-results/threshold/all_corpora_full.csv}
...
printf "%-30s %-12s %7s %9s %9s %9s\n" corpus domain rows "t=0.001" "t=0.01" "t=0.1"
while read -r n tot dom; do
  [ -z "$n" ] && continue
  b=$(fb "$n") || continue
  read -r u bpr <<< "$(.venv/bin/python -c "
import struct,os
f=open('$b','rb'); f.read(8); _,nr,nb,_=struct.unpack('<IIII',f.read(16))
print(nb, max(1,int(os.path.getsize('$b')*3/max(1,nr))))")"
  r=$(( BUDGET / bpr )); [ $r -gt $MAXROWS ] && r=$MAXROWS
  [ $r -gt $tot ] && r=$tot; [ $r -lt 48 ] && r=48
  s=$(( tot / r )); [ $s -lt 1 ] && s=1
  line=$(printf "%-30s %-12s %7s" "$n" "$dom" "$r")
  for t in 0.001 0.01 0.1; do
    v=$(./build_portable/bench_threshold --file "$b" --rows $r --stride $s --t $t \
          --repeats 3 --sparse --csv --tag "$n" 2>/dev/null)
